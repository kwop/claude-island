...
def main():
    ...
    if event == "UserPromptSubmit":
        ...
        # Check if this was a rejection/escape (tool result indicates user refused)
        tool_result = data.get("tool_result", {})
        stop_reason = data.get("stop_reason", "")

        # Detect user rejection (Escape pressed during AskUserQuestion or similar)
        if isinstance(tool_result, dict):
            result_content = str(tool_result.get("content", ""))
            if "user doesn't want to proceed" in result_content.lower() or \
               "rejected" in result_content.lower() or \
               "cancelled" in result_content.lower():
                state["status"] = "rejected"
                state["rejection_reason"] = result_content
            else:
                state["status"] = "waiting_for_input"
        elif stop_reason == "user_rejection":
            state["status"] = "rejected"
        else:
            state["status"] = "waiting_for_input"
